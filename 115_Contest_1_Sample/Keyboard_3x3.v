module Keyboard_3x3(
	...
	input  wire [2:0] column, // 3x3 無段式開關 行(Column)
	output reg  [2:0] row,    // 3x3 無段式開關 行(Column)
	output reg  Pressed,      // 按鍵按下
	output reg  [3:0] KEY     // 按鍵數值
);
...
reg [5:0] release_cnt; // 處理放開彈跳的計數器
reg [3:0] latched_key;
...
always @(posedge clk or negedge rst_n) begin
	if (!rst_n) begin
		...
		// 只在每次掃描點 (1ms) 進行狀態判定
		if (scan_cnt == 16'd10_000) begin
			if (raw_hit) begin
				latched_key <= raw_key;  // 暫存被按下的鍵值
				release_cnt <= 6'd30; // 刷新「放開保護期」(30ms)
				
				if (debounce_cnt < 5'd5) begin // 只要連續命中 5 次有效掃描（約 15ms）
