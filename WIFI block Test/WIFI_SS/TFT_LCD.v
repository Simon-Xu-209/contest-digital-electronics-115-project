...
// 畫面顯示專用的動態暫存器（歸主時脈 clk 統一管理）
reg [7:0] C_hundred, C_ten, C_one;
reg [7:0] L_ten, L_one;

// 產生邊緣偵測訊號：捕捉 clk_cnt[22] 從 0 變 1 的瞬間
...
always @(posedge clk or negedge rst_n) begin
	if(!rst_n) begin
		...
		clk_cnt <= clk_cnt + 1;
		// 使用 count_pulse，確保符合時脈觸發且「只被加一次」
		if((S[15] == 1 || WiFi_signal == 16'd15) && count_pulse) begin
...
// 可調參數配置
// =========================================================================
reg [3:0] SCALE; // 放大倍率 (1 = 8x16, 2 = 16x32, 4 = 32x64)
...
reg [3:0] display_mode; // 顯示模式
reg [9:1] clear_mask;   // 文字清除遮罩
reg [1:0] color_select; // 文字顏色選擇
...
always @(posedge clk or negedge rst_n) begin
	if(!rst_n) begin
		...
		if(switch_8bit[2:0] == 3'b1_00) begin
			if (S[15:1] == 15'b0 && S[0] == 1'b1 || WiFi_signal == 16'd0) begin // 切換為模式 1
				display_mode <= 4'd1;
				TEXT_COLOR <= 16'h001F; // 藍色
			end
			else if (S[15:13] == 3'b0 && S[12] == 1'b1 && S[11:0] == 12'b0 || WiFi_signal == 16'd12) begin // 切換為模式 2
				display_mode <= 4'd2;
			end
			else if(S[15] == 1'b0 && S[14] == 1'b1 && S[13:0] == 14'b0 || WiFi_signal == 16'd14) begin // 文字顏色設定為紅
				TEXT_COLOR <= 16'hF800;
			end
		end
		else if(switch_8bit[2:0] == 3'b1_10) begin // 計數模式畫面
			if(switch_8bit[5:3] != 3'b001) begin // 切換為模式 3
				display_mode <= 4'd3;
				TEXT_COLOR <= 16'h002F; // 藍色
			end else begin
				display_mode <= 4'd4; // 切換為模式 4
			end
		end
	end
end
...
always@(posedge clk) begin
	if(display_mode == 4'd0) begin // 模式 0 預設文字紫色，且顯示 IDLE... 動畫
		SCALE <= 4'd1;
	end
	else if(display_mode == 4'd1) begin // 模式 1 設定數字九宮格畫面，文字顏色由按鍵輸入決定
		SCALE <= 4'd3;
		if(S[15:1] == 15'b0 && S[0] == 1'b1 || WiFi_signal == 16'd0)begin
			clear_mask <= 9'b0;
		end else begin
			if (S[1] || WiFi_signal == 16'd1) clear_mask[1] <= 1'b1;
			if (S[2] || WiFi_signal == 16'd2) clear_mask[2] <= 1'b1;
			if (S[3] || WiFi_signal == 16'd3) clear_mask[3] <= 1'b1;
			if (S[4] || WiFi_signal == 16'd4) clear_mask[4] <= 1'b1;
			if (S[5] || WiFi_signal == 16'd5) clear_mask[5] <= 1'b1;
			if (S[6] || WiFi_signal == 16'd6) clear_mask[6] <= 1'b1;
			if (S[7] || WiFi_signal == 16'd7) clear_mask[7] <= 1'b1;
			if (S[8] || WiFi_signal == 16'd8) clear_mask[8] <= 1'b1;
			if (S[9] || WiFi_signal == 16'd9) clear_mask[9] <= 1'b1;
		end
	end
	else if(display_mode == 4'd2) begin // 模式 2 設定純文字畫面
		SCALE <= 4'd2;
	end
	else if(display_mode == 4'd3 || display_mode == 4'd4) begin // 模式 3 設定計數模式畫面，模式 4 開始計數
		SCALE <= 4'd2;
	end
end
...
always@(*) begin
	...
	case (grid_row)
		8'd0: begin // 第一列
			case (grid_col)
				8'd0: begin
					if(display_mode == 4'd0)begin // 閒置畫面
						current_ascii = "I";
					end
					else if(display_mode == 4'd1) begin // 數字九宮格
					...
					else if(display_mode == 4'd2) begin // 純文字畫面
						current_ascii = "C";
					end
					else if(display_mode == 4'd3 || display_mode == 4'd4) begin // 計數器畫面
						current_ascii = "M";
					end
				end
				8'd1: begin
				...
				8'd4: begin
					if(display_mode == 4'd0 && clk_cnt[25:24] >= 2'b01)begin // 閒置畫面
						current_ascii = ".";
					end
					else if(display_mode == 4'd1) begin // 數字九宮格
				...
				8'd5: begin
					if(display_mode == 4'd0 && clk_cnt[25:24] >= 2'b10)begin // 閒置畫面
						current_ascii = ".";
					end
					else if(display_mode == 4'd2) begin // 純文字畫面
						current_ascii = "D";
					end
					else if(display_mode == 4'd4) begin // 計數器畫面
						current_ascii = "X";
					end
				end
				8'd6: begin
					if(display_mode == 4'd0 && clk_cnt[25:24] >= 2'b11)begin // 閒置畫面
						current_ascii = ".";
					end
					else if(display_mode == 4'd4) begin // 計數器畫面
						current_ascii = "X";
		...
		8'd1: begin // 第二列
			case (grid_col)
				8'd0: begin
					if(display_mode == 4'd1) begin // 數字九宮格
						if(clear_mask[4])begin
							current_ascii = " ";
						end else begin
							current_ascii = "4";
						end
		...
		8'd2: begin // 第三列
			case (grid_col)
				8'd0: begin
					...
					if(display_mode == 4'd3 || display_mode == 4'd4) begin // 計數器畫面
						current_ascii = "C";
					end
				end
				8'd1: begin
					if(display_mode == 4'd2) begin // 純文字畫面
						current_ascii = "O";
					end
				end
				8'd2: begin
					if(display_mode == 4'd1) begin // 數字九宮格
						if(clear_mask[8])begin
							current_ascii = " ";
						end else begin
							current_ascii = "8";
						end
		...
		8'd4: begin // 第五列顯示
			case (grid_col)
				...
				8'd7:begin
					if(display_mode == 4'd4) begin // 計數器畫面
						current_ascii = L_one;
					end
				end
				default: current_ascii = " ";
