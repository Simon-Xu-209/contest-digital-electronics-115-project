...
wire [MAX_RX_LEN-1:0] match_data;
reg [5:0] match_data_pos;
reg [31:0] detected_Data;
reg data_found;

generate
	for (g = 1; g < MAX_RX_LEN-4; g = g + 1) begin : MATCH_GEN
		assign match_data[g-1] = (bytes[g+4] == "D") &&
										 (bytes[g+3] == "a") &&
										 (bytes[g+2] == "t") &&
										 (bytes[g+1] == "a") &&
										 (bytes[g]   == ":");
...
always @(*) begin
	data_found    = 1'b0;
	detected_Data = 8'd0;
	for (k = MAX_RX_LEN - 1; k >= 8; k = k - 1) begin
		if (match_data[k] && !data_found) begin
			data_found = 1'b1;
			detected_Data = {bytes[k-5],
								  bytes[k-6],
								  bytes[k-7],
								  bytes[k-8]};
		end
...
reg rx_ready_reg1, rx_ready_reg2;
always@(posedge clk) begin
	if (!rst_n) begin
		rx_ready_reg1 <= 0;
		rx_ready_reg2 <= 0;
	end else begin
		rx_ready_reg1 <= rx_ready;
		rx_ready_reg2 <= rx_ready_reg1;
	end
end
	...
	if (!rst_n) begin
		...
		case(current_display_state)
			...
			display_EDIT: begin
				...
				char_ascii[4] <= detected_Data[31:24];
				char_ascii[5] <= detected_Data[23:16];
